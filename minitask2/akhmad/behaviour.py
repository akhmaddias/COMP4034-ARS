...
class Behaviour():
    ...
    def turn_left(self):
        ...
        print("Avoiding obstacle by turning left")
    ...
    def turn_right(self):
        ...
        print("Avoiding obstacle by turning right")

    def right_hand_wall_follow(self):
        global is_wall_found

        if not is_wall_found:
            if all(i >= 0.5 for i in front):
                self.publish_command(move_cmd)
            else:
                is_wall_found = True
                self.publish_command(Twist())
        else:
            if min(right) <= 0.5 and min(front) <= 0.5:
                self.publish_command(left_turn_cmd)
                print("Obstacles on front and right - turning left")
            elif min(right) <= 0.5 and min(front) >= 0.5:
                self.publish_command(move_cmd)
                print("No obstacles in front but on right - move forward")
            elif min(right) >= 0.5 and min(front) <= 0.5:
                self.publish_command(left_turn_cmd)
                print("No obstacles on right but in front - turning left")
            elif min(right) >= 0.5 and min(front) >= 0.5:
                if min(right_front) <= 0.5:
                    self.publish_command(right_turn_cmd)
                else:
                    self.publish_command(move_cmd)
                print("No obstacles on right and in front - sliding right")
    ...
    def obstacle_avoidance(self):
        ...
        if continue_right:
            ...
            self.turn_left()
        elif all(i >= 0.5 for i in left_front):
            self.turn_left()
        elif all(i >= 0.5 for i in right_front):
            self.turn_right()
        elif all(i >= 0.5 for i in left_back):
            self.turn_left()
        elif all(i >= 0.5 for i in right_back):
            self.turn_right()
        else:
            self.turn_left()
